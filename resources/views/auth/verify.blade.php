...
@section('content')
    <div class="container-fluid text-center my-3 py-3">
        ...
        <div>
            ...
            @endif
            <p>{{ __('Vous allez recevoir un email de confirmation pour accéder à votre profil sur :') }}
            ...
            <form class="d-inline mb-0" method="POST" action="{{ route('verification.send') }}">
                @csrf
                <button type="submit"
                    class="btn p-0 m-0 align-baseline">{{ __('Cliquez ici pour en recevoir un nouveau') }}</button>.
            </form>
        </div>
    </div>
@endsection
